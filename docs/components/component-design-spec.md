...
> **個別元件規格**:見 `docs/components/` 資料夾,例如 [`BaseButton.md`](./BaseButton.md)。
...
| **⚠️ defineModel 禁止「寫後讀」** | 父層綁 v-model 時 model 是**受控 prop**:寫 `model.value` 只會 emit,**不會同步回讀**(要等父層 flush 才回流)。同一輪執行內需要「寫入後的目前值」,一律以函式回傳值 / 本地變數傳遞,MUST NOT 寫後回讀 `model.value`。僅傳 `modelValue` prop、無 update 監聽的測試走不到受控路徑,抓不到這類 bug(見驗收 checklist)。實例:BaseInputNumber 步進基準 / 長按連發到界判斷、useComposingModel 的 change 正規化,皆曾因寫後讀在受控模式下出錯 |
